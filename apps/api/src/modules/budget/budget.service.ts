...
export class BudgetService {
  ...
  async getSummary(projectId: string) {
    ...
    return {
      ...
      remaining:
        budgetTotal !== null
          ? Math.round((budgetTotal - totalSpent) * 100) / 100
          : null,
      percentUsed: budgetTotal
  ...
  async remove(id: string, actor: JwtPayload) {
    const entry = await this.prisma.budget_entries.findUnique({
      where: { id },
    });
  ...
  private format(e: {
    ...
    return {
      ...
      createdBy: e.creator
        ? { id: e.creator.id, fullName: e.creator.full_name }
        : null,
    };
  }
}
